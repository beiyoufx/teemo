                    ...
                    <div class="table-wrap">
                        <div class="table-container">
                            <div class="btn-group hidden-xs" id="toolbar" role="group">
                                <shiro:hasPermission name="sys:dynamicProperty:create">
                                <button type="button" class="btn btn-outline btn-primary" onclick="addPermission()">
                                    <i class="glyphicon glyphicon-plus" aria-hidden="true"></i><span> 新增属性</span>
                                </button>
                                </shiro:hasPermission>
                            </div>
                            <table id="paginationTable" data-toggle="table" data-mobile-responsive="true" data-height="600" data-icon-size="outline" data-toolbar="#toolbar">
                                ...
                                <tr>
                                    ...
                                    <th data-field="dynamicPropertyKey">属性关键字</th>
                                    <th data-field="dynamicPropertyValue">属性值</th>
...
<script>
    tableModel = {
        ...
        url : "${ctx}/sys/property/find",
        addUrl: "${ctx}/sys/property/add",
        deleteUrl : "${ctx}/sys/property/delete",
        editUrl: "${ctx}/sys/property/edit"
    };
    ...
    function optionFormatter(value, row, index) {
        var editHtml = "&nbsp;<shiro:hasPermission name="sys:dynamicProperty:update"><button type='button' class='fa fa-edit btn btn-primary' onclick='editProperty(" + row.id + ")'>编 辑</button></shiro:hasPermission>&nbsp;";
        var deleteHtml = "&nbsp;<shiro:hasPermission name="sys:dynamicProperty:delete"><button type='button' class='delete fa fa-times btn btn-default' onclick='commonDelete(" + row.id + ")'>删 除</button></shiro:hasPermission>&nbsp;";
    ...
    function editProperty(propertyId) {
        ...
        parent.layer.open({
            type: 2,
            title: ['编辑属性', 'font-weight:bold;'],
            shadeClose: true,
            shade: 0.8,
            area: ['50%', '50%'],
            content: tableModel.editUrl + "/" + propertyId //iframe的url
        });
    }
</script>
